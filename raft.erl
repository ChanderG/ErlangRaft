-module(raft).
-export([start_raft_member/1,start_raft_members/1,append_log/3,make_leader/1,raft_node/1]).
-include_lib("eunit/include/eunit.hrl").

% used to store the state of a single Raft node
-record(ns, {name,
	     log = [],
	     term = 0,
	     commitindex = 0,
...
% all incoming messages are expected to be of the form:
%  {opcode, sender, <args>}
% all outgoing messages are to be of the form:
%  {opcode, Name, response contents}

raft_node(NS) ->
    io:format("Log heartbeat ~n"),
    % commonly used node state fields
    Name = NS#ns.name,
    receive
	{getlog, Sender} ->
	    Sender ! {getlog, Name, NS#ns.log};
	{appendlog, _, LogEntry} ->
	    Log = NS#ns.log,
	    raft_node(NS#ns{log = Log ++ [LogEntry]});
	{disable, Sender} ->
	    Sender ! {disable, Name},
	    raft_node_disabled_mode(Name);
	{getterm, Sender} ->
	    Sender ! {getterm, Name, NS#ns.term};
...
% block until enabled again
raft_node_disabled_mode(Name) ->
    receive
	{enable, Sender} ->
	    Sender ! {enable, Name};
	% ignore all sorts of messages
	_ ->
	    raft_node_disabled_mode(Name)
    end.

% raft_node_append_entries
% returns the updated NS

% if leader term is earlier than currentterm
raft_node_append_entries(NS, Sender, {Term, _, _, _, _}) when Term < NS#ns.term ->
    Sender ! {appendentries, NS#ns.name, NS#ns.term, false},
    NS;
% else
raft_node_append_entries(NS, Sender, {Term, PrevLogIndex, PrevLogTerm, Entries, LeaderCommit}) ->
    ...
    OurPrevLogTerm = if PrevLogIndex == 0 ->
			...
			length(NS#ns.log) == 0 ->
			    0;
			PrevLogIndex > length(NS#ns.log) ->
			    0;
			true ->
			    element(1, lists:nth(PrevLogIndex, NS#ns.log))
		     end,
    if OurPrevLogTerm =/= PrevLogTerm ->
       ...
       true ->
	    ...
	    NewCommitIndex = if LeaderCommit > NS#ns.commitindex -> min(LeaderCommit, length(NewLog));
				true -> NS#ns.commitindex
			     end,
...
start_raft_member(UniqueId) ->
    register(UniqueId, spawn(raft, raft_node, [#ns{name=UniqueId}])).
...
append_log(Id,Num,Something) ->
    Id ! {appendlog, self(), {Num, Something}}.
...
get_term(Id) ->
    Id ! {getterm, self()},
    receive
	{getterm, Id, Term} -> Term
    end.

get_commit_index(Id) ->
    Id ! {getcommitindex, self()},
    receive
	{getcommitindex, Id, CommitIndex} ->
	     CommitIndex
    end.
